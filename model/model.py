...
class CoopNets(nn.Module):
    ...
    def train(self):
        ...
        des_optimizer = torch.optim.Adam(self.descriptor.parameters(), lr=self.opts.lr_des, # theta = descriptor.parameters
                                         betas=[self.opts.beta1_des, 0.999])
        gen_optimizer = torch.optim.Adam(self.generator.parameters(), lr=self.opts.lr_gen, # alpha = generator.parameters
        ...
        for epoch in range(self.opts.num_epoch):
            ...
            for i in range(num_batches):
                ...
                # G0: Y = g(z,alpha)+eps, Y ~ q(z,y,alpha)
                z = torch.randn(self.num_chain, self.opts.z_size, 1, 1)
                z = Variable(z.cuda(), requires_grad=True)
                # NCHW
                gen_res = self.generator(z)

                # D1: tilde Y  Y_tau -> Y_tau+1, use df/dY, Y_0 = gen_res
                if self.opts.langevin_step_num_des > 0:
                    revised = self.langevin_dynamics_descriptor(gen_res)
                # G1: z_tau -> z_tau+1, use d (log q(z,y,alpha)/dz, given y=tilde Y, z_0 white noise
                if self.opts.langevin_step_num_gen > 0:
                    z = self.langevin_dynamics_generator(z, revised)

                # D2
                obs_feature = self.descriptor(obs_data) # f_theta(Ydata_i), i=1..n, n=batchsize
                revised_feature = self.descriptor(revised) # f_theta(tilde Y_i), i=1..m, m=n?
                # L_p def?
                des_loss = (revised_feature.mean(0) - obs_feature.mean(0)).sum() # sum? (1/n sum_i f(Ydata_i) - 1/m sum_i f(tilde Y_i) )
                des_optimizer.zero_grad()
                des_loss.backward()
                des_optimizer.step() # theta -> theta'

                # G2: L_q'(alpha), L_q = E_q(Z|Y,alpha) log q(Z,Y,alpha)  
                ini_gen_res = gen_res.detach()
                if self.opts.langevin_step_num_gen > 0:
                    gen_res = self.generator(z)
                # gen_res=gen_res.detach()
                gen_loss = 0.5 * self.opts.sigma_gen * self.opts.sigma_gen * mse_loss(gen_res,
                                                                                      revised.detach()) #||tilde Y-g(z_tau,alpha)||^2

                gen_optimizer.zero_grad()
                gen_loss.backward()
                gen_optimizer.step() # alpha -> alpha'

                # Compute reconstruction loss
                recon_loss = mse_loss(revised, ini_gen_res) # tilde Y - Y_0 
